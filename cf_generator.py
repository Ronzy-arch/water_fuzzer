...
def generate_cloudflare_clearance():
    ...
    # MEMBACA TARGET PERTAMA DARI FILE TARGET.TXT SECARA DINAMIS
    ...
    try:
        ...
        if "cf_clearance" in cookies:
            ...
            # MEMANIPULASI DAN MENYUNTIKKAN DATA COOKIE SECARA OTOMATIS KE CONFIG.PY
            if os.path.exists(config_path):
                with open(config_path, "r", encoding="utf-8") as f:
                    content = f.read()
                
                if "USER_AGENTS" in content:
                    content = re.sub(r'USER_AGENTS\s*=.*', '', content, flags=re.DOTALL)
